...
| ft_strcmp | `int	ft_strcmp(const char *s1, const char *s2)` | Compares s1 and s2. It returns an integer less than, equal to, or greater than zero if s1 is found. |
...
| ft_strrchrlen | `int	ft_strrchrlen(const char *s, int c)` | Returns the length of the last occurrence of the character c in the string s. |
